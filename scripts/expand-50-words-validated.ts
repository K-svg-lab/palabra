/**
 * Expand Word List by 50 Words with Full Validation
 * Phase 18.1.7 Extension - Test Expansion
 * 
 * This script:
 * 1. Uses Wiktionary top 1000 as source (credible, lemmatized)
 * 2. Filters out words already in database and JSON
 * 3. Takes next 50 unique words
 * 4. Enriches with OpenAI (POS, translation)
...
// Wiktionary top 1000 Spanish words (extended list for expansion)
const WIKTIONARY_TOP_1000 = [
  // Original top 200 (most likely already in database)
  ...
  // Extended: Ranks 201-400 (more likely to be new)
  "crear", "llegar", "pasa", "grandes", "diferencia", "época", "llevar", "edad", "economía",
  "dar", "social", "desarrollo", "papel", "real", "menos", "central", "nacional", "ejemplo",
  ...
  "función", "luz", "empresa", "alto", "continuar", "hombre", "experiencia", "servicio", "social",
  ...
  "familia", "común", "datos", "idea", "veces", "condiciones", "estudiar", "situación", "sentir",
  "permitir", "guerra", "cambiar", "dirección", "oficina", "propios", "ciudad", "natural", "zona",
...
async function getExistingWords(): Promise<Set<string>> {
  ...
  console.log(`   Database: ${dbWords.size} words`);
...
async function expand50Words(): Promise<ExpansionResult> {
  console.log('╔═══════════════════════════════════════════════════════════════╗');
  console.log('║   Expand Word List by 50 Words (Validated)                   ║');
  console.log('║   Phase 18.1.7 Extension - Test Expansion                    ║');
  console.log('╚═══════════════════════════════════════════════════════════════╝\n');
  ...
  // STEP 4: Filter Wiktionary list for new words
  ...
  console.log(`🆕 Found ${newWords.length} new words in Wiktionary list`);
  ...
  if (wordsToAdd.length === 0) {
    ...
    console.log('Consider using a different word source.\n');
  ...
  if (finalNew.length === 0) {
    ...
    console.log('   All words either were duplicates or already existed in list.\n');
    console.log('💡 Consider using a different word source or increasing the source range.\n');
    await prisma.$disconnect();
    return {
      wordsAdded: 0,
  ...
  // Update currentRank for remaining words
  finalNew.forEach((word, index) => {
    word.rank = startingCount + index + 1;
  });

  // STEP 6: VALIDATE NEW WORDS (Checkpoint 1)
  // Note: Skip rank validation for new words batch, will validate full list after merging
  ...
  console.log('   3. Run: npx tsx scripts/pre-generate-vocabulary.ts --resume\n');
